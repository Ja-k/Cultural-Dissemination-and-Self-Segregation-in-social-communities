...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (int iteration = 0; iteration < M; iteration++) {
			...
			/*
			 * List<Node> segregated = new ArrayList<>(); HashMap< Node ,
			 * List<Node> > graph = new HashMap<>(); try{ for(int i = 0 ; i <
			 * square_lattice.length; i++){ for(int j = 0 ; j <
			 * square_lattice.length; j++){ if(square_lattice[i][j] != null){
			 * if(j == 0 && i == 0){ List<Node> neighbors = new ArrayList<>();
			 * if(square_lattice[i+1][j] != null){
			 * neighbors.add(square_lattice[i+1][j]); }
			 * if(square_lattice[i][j+1] != null){
			 * neighbors.add(square_lattice[i][j+1]); } if(neighbors.size() ==
			 * 0){ segregated.add(square_lattice[i][j]); }else{
			 ...
			 * neighbors.add(square_lattice[i][j-1]); }
			 * if(square_lattice[i+1][j] != null){
			 * neighbors.add(square_lattice[i+1][j]); } if(neighbors.size() ==
			 * 0){ segregated.add(square_lattice[i][j]); }else{
			 * graph.put(square_lattice[i][j], neighbors); }
			 * //graph.put(square_lattice[i][j], neighbors);
			 * 
			 * }else if( i == 0 && j == (square_lattice.length-1)){ List<Node>
			 * neighbors = new ArrayList<>(); if(square_lattice[i][j-1] !=
			 * null){ neighbors.add(square_lattice[i][j-1]); }
			 * if(square_lattice[i+1][j] != null){
			 * neighbors.add(square_lattice[i+1][j]); } if(neighbors.size() ==
			 * 0){ segregated.add(square_lattice[i][j]); }else{
			 * graph.put(square_lattice[i][j], neighbors); }
			 ...
			 * //graph.put(square_lattice[i][j], neighbors);
			 * 
			 * }else if( j == (square_lattice.length - 1)&& ((i+1) <=
			 * square_lattice.length-1) ){ List<Node> neighbors = new
			 * ArrayList<>(); if(square_lattice[i-1][j] != null){
			 * neighbors.add(square_lattice[i-1][j]); }
			 * if(square_lattice[i][j-1] != null){
			 * neighbors.add(square_lattice[i][j-1]); }
			 * if(square_lattice[i+1][j] != null){
			 ...
			 * //graph.put(square_lattice[i][j], neighbors);
			 * 
			 * }else if( (i == square_lattice.length - 1)&& ((j+1) <=
			 * square_lattice.length - 1) ){ List<Node> neighbors = new
			 * ArrayList<>(); if(square_lattice[i][j-1] != null){
			 * neighbors.add(square_lattice[i][j-1]); }
			 * if(square_lattice[i][j+1] != null){
			 * neighbors.add(square_lattice[i][j+1]); }
			 * if(square_lattice[i-1][j] != null){
			 ...
			 * //graph.put(square_lattice[i][j], neighbors);
			 * 
			 * }else if((i == (square_lattice.length - 1 ))&&(j ==
			 * (square_lattice.length - 1))){ List<Node> neighbors = new
			 * ArrayList<>(); if(square_lattice[i][j-1] != null){
			 * neighbors.add(square_lattice[i][j-1]); }
			 * if(square_lattice[i-1][j] != null){
			 ...
			 * //graph.put(square_lattice[i][j], neighbors);
			 * 
			 * }else{ List<Node> neighbors = new ArrayList<>();
			 * if(square_lattice[i][j+1] != null){
			 * neighbors.add(square_lattice[i][j+1]); }
			 * if(square_lattice[i][j-1] != null){
			 * neighbors.add(square_lattice[i][j-1]); }
			 ...
			 * //graph.put(square_lattice[i][j], neighbors); }
			 * 
			 * }else continue; } }//End of for loop }catch(Exception e){
			 * e.printStackTrace(); }
			 */

			// System.out.println("The graph size is = " + graph.size());
			// System.out.println("The No. of segregatd nodes  is = " +
			// segregated.size());
			/*
			...
			for (int i = 0; i < NeighborHood.size(); i++) {
				...
				double overlap = cultural_overlap(NeighborHood.get(i).get(0),
						randomNode);
				...
				if (averageOverlap < T) {
					replaceNode(NeighborHood.get(i).get(0), square_lattice, q);
				}
			}
		}// End of Iteration
	}// End of main method
